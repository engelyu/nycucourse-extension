...
有些課同一個課號可以用兩種方式採計，例如醫放系的「計算機概論」可以算系上選修，也可以算核心課程（基本素養-量性推理）。**選課系統在加入預排時就決定了採計方式**，之後正式登記會照這個方式，不能在登記時再改。

所以按「加入」時，如果這門課有兩種採計方式，會先列出來讓你選，例如「選修」或「核心・基本素養-量性推理」。只有一種的課會直接加入。

2026 年 9 月整理全校課程時，大約每 30 門有 1 門有兩種採計方式，沒有超過兩種的；另一種幾乎都是核心課程，少數是語言與溝通。

加錯了可以到「選課」分頁，在課名下方的「採計」旁按「變更」。
...
批次加入沒辦法一門一門問，有兩種採計方式的課會先用開課系所的方式（例如選修）加入，並在結果裡列出另一種可能。要算核心的話，到「選課」分頁按「變更」。
...
3. 設定條件：
   ...
   - 也可以篩校區、類別（必修、選修、核心、語言與溝通）、系所（可複選、可搜尋）、關鍵字；結果可以選排序方式。
4. 滑過一門課，格子上會標出它的上課時間。每門課可以：
   - **加入預排**（有兩種採計方式時先讓你選）。
   - **查詢**：列出每種採計方式能不能選、人數、衝堂或原因；可以選的會有「加選」或「登記」按鈕。還不在預排的課會先自動加入預排，再跳出確認視窗（志願序課程要選第幾志願），按「送出加選」或「送出登記」才會真的送出。
5. 每門課會標出目前狀態（已選上、登記中、在預排），格子也依狀態上色（正式選上、登記中、在預排、私人行程各一色）。狀態會在你加入、加選、登記後自動更新，也可以按上方「從選課網更新狀態」。

選的時段和篩選條件會記住，下次打開還在。要篩核心、語言與溝通，以及用系所找到多系合開的課，需要先在「加入預排」按一次「更新課程資料」。

### 正式選課

點擴充功能的「選課」會開啟選課分頁，列出你的預排課程。

1. 每門課的課名下方會顯示「採計」方式。顯示「未指定」的，是用 0.6.0 以前的版本加入的，選課網自己的「登記」按鈕會失敗；按旁邊的「變更」選好採計方式就好。
2. 按「查詢」，擴充功能會向選課網確認這門課能不能加選，顯示人數、衝堂數與學校給的訊息。
3. 可以加選時按鈕會變成「加選」，按下後要再確認一次才會送出。
4. 體育、通識這類需要志願序的課，確認視窗會列出五個志願、各志願目前登記人數，選好才能送出。
5. 送出結果直接顯示選課網的回覆，成功後該門課會標示「已選上」。
