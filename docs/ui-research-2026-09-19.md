...
| # | 狀態 | 驗證 |
|---|---|---|
| 1 | 完成 | 瀏覽器測試 planner：已套用的篩選列出來、單獨拿掉、清除全部 |
| 2 | 完成 | 單元測試 facetCounts／relaxations；瀏覽器測試選項門數與零結果建議。真實全校資料：每次篩選約 16ms |
| 3 | 完成 | 瀏覽器測試：復原按鈕、⌘Z／Ctrl+Z、全部清除後可以復原 |
| 4 | 完成 | 單元測試格子標記；配色改用 Okabe-Ito，標籤改成淺底深字 |
| 5 | 完成 | 瀏覽器測試：按鈕寫「送出登記」「先不要」，焦點預設在「先不要」 |
| 6 | 完成 | 瀏覽器測試 sticky：捲動 3000px 後格子仍在畫面上，滑過課程時仍有預覽 |
| 7 | 完成 | 瀏覽器測試 keyboard：Tab 只停一次、方向鍵、Space、Shift＋方向鍵、整天、整節，以及 ARIA 標記 |
